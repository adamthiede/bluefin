...
ARG CHROME
RUN	bash -c "if [[ \"${CHROME}\" == \"chrome\" ]];then echo 'INSTALLING CHROME'; mkdir -p /usr/lib/opt/google /var/opt; ln -s /var/opt /opt; ln -s /usr/lib/opt/google /var/opt/google; rpm-ostree override remove firefox firefox-langpacks; sudo rpm-ostree install https://dl.google.com/linux/direct/google-chrome-stable_current_x86_64.rpm; fi" && \
