...
RUN	bash -c "if [[ \"${CHROME}\" == \"chrome\" ]];then mkdir /var/opt; sed -e 's,enabled=0,enabled=1,' -i /etc/yum.repos.d/google-chrome.repo; rpm-ostree override remove firefox firefox-langpacks --install google-chrome-stable; fi" && \
